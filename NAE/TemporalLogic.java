...
public class TemporalLogic extends Converter{
    ...
    /******************************************
     * this function prints the special modules
     * in case of a special node 
     *****************************************/
    String getSpecialModule(Node n,int valueNumber){
        StringBuilder module = new StringBuilder();
        module.append(n.getModuleDeclaration());        
        module.append("VAR\n");
        for(int i =0;i < valueNumber;i++){
        ...
        module.append(n.getConnections());

        module.append(n.getSpecialStateAndValue("main.i",valueNumber));
    ...
    void generateNuSMV(String name)throws Exception{
        ...
        /***************************************************
         * for each node, check if it is special or regular
         *  node and send it to the suitable function
         **************************************************/
        
    	for(Node n:nodes.values()){
    		if(n.functions[0] == 81){
    			code.append(getSpecialModule(n,valueNumber));
    		}
    		else{
    ...
    String getSpecTL(){
        ...
        /************************************************************
         * in case we have a special node, add error check to ltlspec 
         ************************************************************/
        for(Node n:nodes.values()){
        	if(n.functions[0] == 81){
