...
class Node{
    ...
    String getModuleDeclaration(){
        ...
        /***************************************************************************
         * in case we have a special  node we need the node to access module state81 
         **************************************************************************/
    ...
    /***************************************************************************
     * in case we have a special  node we create a new function that gets only 
     * the node connections (without functions) 
     **************************************************************************/  
    ...
    /*****************************************************************
     *in case of special node we duplicate getStateAndValueTransitions
     * and removed all the transitions reference   
     *****************************************************************/
    //freedomExpression is an expression that evaluates to true whenever the state allows the value and ko/fe vars to be free
    //valueNumber is the number of value and corresponding transition vars we have in this module.
    String getSpecialStateAndValue(String freedomExpression,int valueNumber){
        StringBuilder module = new StringBuilder();               
        //if node is KO or FE, allow for transition
        for(int i=0;i<valueNumber;i++){
            String num = valueNumber==1 ? "" : ""+i;
        ...
        for(int valNum = 0;valNum<valueNumber;valNum++){      
            ...
            States statesWriter = new States(this.inputs.toArray(new Input[0]));
            String state = "";
